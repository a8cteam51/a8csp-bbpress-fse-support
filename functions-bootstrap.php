 ...
 */
function a8csp_bbpress_fse_support_get_plugin_metadata( $property = null ) {
	...
	if ( ! isset( $plugin_data[ $translate_key ] ) ) {
		...
		$plugin_file                   = trailingslashit( WP_PLUGIN_DIR ) . constant( 'A8CSP_BBPRESS_FSE_SUPPORT_BASENAME' );
 ...
 * @return  string
 */
function a8csp_bbpress_fse_support_get_plugin_slug() {
	$text_domain = a8csp_bbpress_fse_support_get_plugin_metadata( 'TextDomain' );
 ...
 * @return  string
 */
function a8csp_bbpress_fse_support_get_plugin_name() {
	return a8csp_bbpress_fse_support_get_plugin_metadata( 'Name' );
}
 ...
 * @return  string
 */
function a8csp_bbpress_fse_support_get_plugin_version() {
	return a8csp_bbpress_fse_support_get_plugin_metadata( 'Version' );
}
 ...
 * @return  bool
 */
function a8csp_bbpress_fse_support_is_wp_version_compatible( $min_wp_version ) {
 ...
 * @return  bool
 */
function a8csp_bbpress_fse_support_is_php_version_compatible( $min_php_version ) {
 ...
 */
function a8csp_bbpress_fse_support_validate_requirements() {
	$plugin_metadata = a8csp_bbpress_fse_support_get_plugin_metadata();
	...
	$is_php_compatible = a8csp_bbpress_fse_support_is_php_version_compatible( $plugin_metadata['RequiresPHP'] );
	$is_wp_compatible  = a8csp_bbpress_fse_support_is_wp_version_compatible( $plugin_metadata['RequiresWP'] );
 ...
 * @return  void
 */
function a8csp_bbpress_fse_support_output_requirements_error( $error ) {
	add_action(
		'admin_notices',
		static function () use ( $error ) {
			$requirements_error = \wp_sprintf(
				/* translators: 1: Plugin name, 2: Plugin version */
				__( '<strong>%1$s (version %2$s)</strong> could not be initialized.', 'a8csp-bbpress-fse-support' ),
				a8csp_bbpress_fse_support_get_plugin_metadata( 'Name' ),
				a8csp_bbpress_fse_support_get_plugin_metadata( 'Version' )
			);

			if ( $error->has_errors() ) {
				$requirements_error .= ' ' . \__( 'Your environment does not meet all the system requirements listed below:', 'a8csp-bbpress-fse-support' );
					...
					switch ( $error_code ) {
						case 'plugin_wp_incompatible':
							$error_message = wp_sprintf(
								...
								__( 'Current <em>WordPress version (%1$s)</em> does not meet minimum required version of %2$s.', 'a8csp-bbpress-fse-support' ),
						...
						case 'plugin_php_incompatible':
							$error_message = wp_sprintf(
								...
								__( 'Current <em>PHP version (%1$s)</em> does not meet minimum required version of %2$s.', 'a8csp-bbpress-fse-support' ),
								PHP_VERSION,
								$error_data['requires_php']
							);
							break;
						case 'missing_autoloader':
							$error_message = __( 'The autoloader file is missing. Please run <code>composer install</code> to generate it.', 'a8csp-bbpress-fse-support' );
							break;
						default:
